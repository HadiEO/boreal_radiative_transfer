...
#  if(MCmethod == "sobolEff") x0 <- sobolEff(model=NULL, X1, X2, ...)
  ...
  if (!is.null(model)){
    y <- model(x0$X, ...)       # !!! This is the bottleneck: this calls 'model' and evaluate all X samples, maybe parallelize here i.e. in the sobol function definition!
    # y_nonpar <- y
    # y_par <- y
    y_multi <- y                # Ask to return model output y as vector, see return()
    V <- 0
    for (i in 1:q){
      tell(x0,y[,i])
      V <- V + x0$V[1]
  ...
  x$y_multi <- y_multi                  # Attach y_multi (model multioutput)
  return(x)
