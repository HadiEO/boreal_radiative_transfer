paras.interm.varymaininput.wL <- function(LAIe, i0, cgf.view,   # Gap fractions data from DHP
  ...
  LAIt <- 0.5078 + 1.0199 * LAIe                                               # True LAI
  
  DIFN = exp(-0.7891 * LAIe)                                        # Empirical fit, form proposed in Rautiainen 2009
  ...
  out <- c(BRF, LAIt, DIFN, p, q, wC, Q)   # <======================= return as appended row-vector so .combine = "rbind"
  return(out)
}